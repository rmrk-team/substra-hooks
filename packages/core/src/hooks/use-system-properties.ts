...
export const useSystemProperties = (apiProviderId?: string): ISystemProperties | null => {
  const id = apiProviderId || useContext(SubstraHooksContext).defaultApiProviderId;
  return useContext(SubstraHooksContext).apiProviders[id]?.systemProperties || null;
};
